...
-- Structured content sections on listings (display-only; features[] stays for filtering).
ALTER TABLE public.listings
  ADD COLUMN IF NOT EXISTS content_sections jsonb NOT NULL DEFAULT '[]'::jsonb;

-- Site-wide "primary agent" fields shown on the listing detail agent block
-- until a fully staffed profiles/user_roles setup is in place per client.
ALTER TABLE public.site_settings
  ADD COLUMN IF NOT EXISTS primary_agent_name text,
  ADD COLUMN IF NOT EXISTS primary_agent_role text,
  ADD COLUMN IF NOT EXISTS primary_agent_photo_url text;

-- Rebuild the public view chain so content_sections is exposed to anon.
